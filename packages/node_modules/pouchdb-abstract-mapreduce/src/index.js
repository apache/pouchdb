...
function createAbstractMapReduce(localDocName, mapper, reducer, ddocValidator) {
  ...
  async function getDocsToPersist(docId, view, docIdsToChangesAndEmits) {
    ...
    function getMetaDoc() {
      ...
      return defaultsTo(view.db.get(metaDocId), defaultMetaDoc);
    }
  ...
  async function updatePurgeSeq(view) {
    ...
    try {
      ...
      const rev = await defaultsTo(view.db.get('_local/purgeSeq'), undefined, '_rev');
  ...
  async function saveKeyValues(view, docIdsToChangesAndEmits, seq) {
    ...
    const lastSeqDoc = await defaultsTo(view.db.get(seqDocId), {_id: seqDocId, seq: 0});
  ...
  async function updateViewInQueue(view, opts) {
    ...
    async function getRecentPurges() {
      const purgeSeq = await defaultsTo(view.db.get('_local/purgeSeq'), -1, 'purgeSeq');

      const purges = await defaultsTo(view.sourceDB.get('_local/purges'), [], 'purges');
      ...
      return Promise.all(uniquePurges.map(async (docId) => {
          const doc = await defaultsTo(view.sourceDB.get(docId), undefined);
